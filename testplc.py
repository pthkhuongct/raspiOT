...
@login_required
def index():
    return render_template('index.html', voltage_db3=voltage_db3, voltage_db4=voltage_db4)

@app.route('/data')
@login_required
def data():
    return jsonify(voltage_db3=voltage_db3, voltage_db4=voltage_db4)

def read_plc_values():
    global voltage_db3, voltage_db4
    ...
    while True:
        try:
            seft_db3 = plc.db_read(3, 0, 4)
            voltage_db3 = snap7.util.get_real(seft_db3, 0)

            seft_db4 = plc.db_read(4, 0, 4)
            voltage_db4 = snap7.util.get_real(seft_db4, 0)

            print(f"Đọc DB3: {voltage_db3} V, DB4: {voltage_db4} V")
        except Snap7Exception as e:
...
def update_excel():
    ...
    if not os.path.exists(file_path):
        ...
        sheet.title = "Dữ liệu điện áp"
        sheet.append(["Thời gian", "Điện áp đồng hồ 1 (V)", "Điện áp đồng hồ 2 (V)"])
    ...
    while True:
        try:
            if os.path.exists(file_path):
                ...
                sheet.append([timestamp, voltage_db3, voltage_db4])
                workbook.save(file_path)
